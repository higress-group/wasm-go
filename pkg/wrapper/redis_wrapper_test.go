...
// TestPasswordURLEncoding verifies that special characters in Redis passwords
...
func TestPasswordURLEncoding(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			result := url.QueryEscape(c.input)
			assert.Equal(t, c.expected, result, "URL encoding mismatch for input: %s", c.input)
		})
	}
}
...
func TestUsernameURLEncoding(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			result := url.QueryEscape(c.input)
			assert.Equal(t, c.expected, result, "URL encoding mismatch for username: %s", c.input)
		})
	}
}
